// Copyright (c) 2025 The Sqoin Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries sqoin-node, sqoin-gui.
  -M, --monolithic       Run monolithic binaries sqoind, sqoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'sqoin-qt [ARGS]' or 'sqoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'sqoind [ARGS]' or 'sqoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'sqoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'sqoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'sqoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_sqoin [ARGS]'.
  chainstate [ARGS] Run sqoin kernel chainstate util, equivalent to running 'sqoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_sqoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_sqoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "sqoin-gui" : "sqoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "sqoin-node" : "sqoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("sqoin-cli");
            // Since "sqoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("sqoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("sqoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_sqoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("sqoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_sqoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_sqoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("sqoin-util");
        } else {
...
//! Execute the specified sqoind, sqoin-qt or other command line in `args`
...
//!             be a relative path to a sqoind, sqoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/sqoin ...
